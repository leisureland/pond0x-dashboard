  ...
  const fetchWalletData = async (ethAddr?: string, solAddr?: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Include the full response with pond0xData
        setStats({
          ...data.stats,
          pond0xData: data.pond0xData
        });
        
        toast({
          title: "Data Loaded",
          description: `Fetched ${data.events?.length || 0} transactions from blockchain APIs`,
        });
      } else {
        console.error('Blockchain API failed:', response.status, response.statusText);
        setEvents([]);
        setStats(null);
        
        toast({
          title: "Blockchain API Error", 
          description: `Failed to fetch data (${response.status}). Please check your wallet address and try again.`,
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Network Error",
        description: "Failed to connect to blockchain APIs. Please check your internet connection.",
        variant: "destructive"
      });
    } finally {
      setIsLoading(false);
